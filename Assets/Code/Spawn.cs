...
public class Spawn : MonoBehaviour
{
    ...
    void Update()
    {
        if(timeBtwSpawn <= 0) {
            if(spawnSide == 0) {
                Instantiate(banana, new Vector2(34f, Random.Range(-20f, 20f)), Quaternion.identity);
                ...
                Instantiate(banana, new Vector2(Random.Range(-17f, 17f), -16f), Quaternion.identity);
                banana.transform.localScale = new Vector2(spawnSize, spawnSize);
                spawnSize = sizes[Random.Range(1, sizes.Length)];
                spawnSide = Random.Range(0, 4);
            }
